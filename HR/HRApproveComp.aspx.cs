...
{
    public partial class HRApproveComp : Page
    {
            ...
            using (SqlConnection conn = new SqlConnection(cs))
            {
                string query = @"
                    SELECT 
                        ...
                        l.date_of_request,
                        l.start_date,        -- compensation date
                        l.num_days,
                        l.final_approval_status
                    FROM dbo.Compensation_Leave cl
                    INNER JOIN dbo.[Leave] l ON cl.request_ID = l.request_ID
                    INNER JOIN dbo.Employee e ON cl.emp_ID = e.employee_id
        ...
        private void HandleAction(bool approve)
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(cs))
                {
                    ...
                    else
                    {
                        using (SqlCommand cmd = new SqlCommand(@"
                            UPDATE Employee_Approve_Leave
                                SET status = 'Rejected'
                            WHERE Emp1_ID = @hr AND leave_ID = @req;

                            UPDATE [Leave]
                                SET final_approval_status = 'Rejected'
                            WHERE request_ID = @req;", conn))
                        {
                            cmd.Parameters.AddWithValue("@hr", hrId);
                    ...
                    lblResult.Text = "Compensation leave processed. Refresh the list to see updated status.";
                }
