...
{
    public partial class HRApproveAnAcc : System.Web.UI.Page
    {
        ...
        SELECT
            ...
            CASE
                WHEN EXISTS (SELECT 1 FROM Annual_Leave al WHERE al.request_ID = l.request_ID) THEN 'Annual'
                WHEN EXISTS (SELECT 1 FROM Accidental_Leave ac WHERE ac.request_ID = l.request_ID) THEN 'Accidental'
            END AS leave_type
        FROM dbo.[Leave] l
        WHERE l.final_approval_status = 'Pending'
          AND (
                EXISTS (SELECT 1 FROM dbo.Annual_Leave al WHERE al.request_ID = l.request_ID)
             OR EXISTS (SELECT 1 FROM dbo.Accidental_Leave ac WHERE ac.request_ID = l.request_ID)
          )
        ...
        private bool RequestExists(int requestId, SqlConnection conn)
        {
            using (SqlCommand checkCmd = new SqlCommand(
                "SELECT COUNT(*) FROM [Leave] WHERE request_ID = @id", conn))
            {
